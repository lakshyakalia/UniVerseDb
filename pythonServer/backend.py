...
def vendorDetailU2(vendorDetails, itemsId, recordID):
    ...
    vendorArray.insert(3, 0, 0, bytes(vendorDetails['Street'], "utf-8") + u2py.VM + bytes(vendorDetails['City'],
                                                                                          "utf-8") + u2py.VM + bytes(
        vendorDetails['State'], "utf-8") + u2py.VM + bytes(vendorDetails['Zip'], "utf-8"))
...
def writePurchaseOrder(purchaseOrderDetails, itemOrderDetails, recordID, submitStatus):
    itemID = quantity = cost = ap=bytes("", "utf-8")
    ...
    for item in itemOrderDetails:
        ...
        ap=ap+bytes(str(0), "utf-8") + u2py.VM
...
def saveNewOrder():
    ...
    writePurchaseOrder(data['purchaseOrderDetails'], data['itemOrderDetails']['SpecialRequests'], recordID,
                       data['submitStatus'])
    return {
        'status': 200,
...
def editParticularOrder(orderID):
    data = request.get_json()
    writePurchaseOrder(data['purchaseOrderDetails'], data['itemOrderDetails']['SpecialRequests'], orderID,
                       data['submitStatus'])
    return {
        'status': 200,
...
def allInvoice():
    ...
    invoiceDetails = []
    invoice = {}
    # invoiceDetails = json.dumps(json.loads(data))
...
def invoiceCreate():
    data = request.get_json()
    saveInvoice(data['invoiceDetails']['orderNo'], data['invoiceDetails']['invoiceDetails'],
